  ...
  try {
    ...
    if (!mimeType || !supportedTypes.includes(mimeType)) {
      return Response.json(
        {
          error:
            "Unsupported file type. Please use PNG, JPEG, GIF, WebP, or PDF.",
        },
        { status: 400 }
      );
    }
    ...
    if (mimeType.startsWith("image/") && base64Data.length < 100) {
      return Response.json(
        {
          error:
            "Invalid image format. Please ensure the image is not corrupted and is in a supported format.",
        },
        { status: 400 }
      );
    }
      ...
      async start(controller) {
        try {
          ...
          const systemPrompt =
            ...
            "Analyze this prescription image and provide detailed information about medications, dosages, and instructions.";
...
3. Get detailed AI analysis`;
    ...
    return Response.json(
      {
        error:
          "Failed to process file. Please try again with a different image.",
      },
      { status: 500 }
    );
  }
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const messages = [new SystemMessage(MEDICAL_SYSTEM_PROMPT)];
